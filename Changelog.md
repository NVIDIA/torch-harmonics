...
* **Breaking**: the spherical harmonic transforms and the quadrature layer now take a `GridS2` descriptor in place of a resolution and a grid name. `RealSHT(nlat, nlon, lmax=..., grid="legendre-gauss")` becomes `RealSHT(as_grid("legendre-gauss", (nlat, nlon)), lmax=...)`, and likewise for `InverseRealSHT`, `RealVectorSHT`, `InverseRealVectorSHT`, `QuadratureS2` and their distributed counterparts. The descriptor carries the resolution, so `nlat`/`nlon`/`img_shape` are gone from those signatures; both remain available as attributes on the module. Passing the old arguments raises a `TypeError` naming the replacement rather than failing deep inside the constructor.
