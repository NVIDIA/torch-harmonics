...
* **Breaking**: the default `theta_cutoff` of `DiscreteContinuousConvS2`, `DiscreteContinuousConvTransposeS2`, `NeighborhoodAttentionS2`, and their distributed counterparts is now derived from the grid's actual latitude spacing rather than from `nlat` alone. The previous expression was exact only for equiangular grids and under-covered the poles on the others, silently degenerating the stencil there. Equiangular grids, which are the default, are unaffected in practice, but `"lobatto"` widens by ~21%, `"equiangular-trapezoidal"` by ~5x, and `"legendre-gauss"` narrows by ~2.3%. Models trained on non-equiangular grids will see a different stencil unless `theta_cutoff` is passed explicitly; a `UserWarning` is emitted on the grids whose default changed.
* **Breaking**: fixed the `"bilinear-spherical"` resampling mode of `ResampleS2` and `DistributedResampleS2`, which applied spherical interpolation weights that are only valid for vectors to scalar sample values. The mode was not shift-equivariant and could amplify without bound, mapping inputs in `[-pi, pi]` to outputs spanning roughly `[-12600, 15200]`. It now interpolates along the shorter arc of the circle, which is identical to `"bilinear"` unless the field contains a phase wrap, so only wrapped fields change. The previous behaviour was effectively untested, since the only case covering the mode used a constant field.
