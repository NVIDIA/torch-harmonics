...
* **Breaking**: the default `theta_cutoff` of `DiscreteContinuousConvS2`, `DiscreteContinuousConvTransposeS2`, `NeighborhoodAttentionS2`, and their distributed counterparts is now derived from the grid's actual latitude spacing rather than from `nlat` alone, which was exact only for equiangular grids and under-covered the poles on the others. Equiangular grids are unaffected; `"lobatto"` widens by ~21%, `"legendre-gauss"` narrows by ~2.3%, and for `"equiangular-trapezoidal"` the widening grows with resolution. Models trained on non-equiangular grids will differ unless `theta_cutoff` is passed explicitly.
...
* The SHT classes now emit a `UserWarning` on an `"equiangular-trapezoidal"` grid instead of raising `ValueError("Unknown quadrature mode")`. That rule converges only algebraically, so the associated Legendre polynomials are not discretely orthogonal and the transform does not round-trip: at `nlat = 64` the relative error is 2e-2 at `lmax = 8` and 6.7e-1 at the default `lmax = 32`. The grid remains appropriate for plain quadrature and for the localized operators.
