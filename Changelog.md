...
* **Breaking**: the default `theta_cutoff` of `DiscreteContinuousConvS2`, `DiscreteContinuousConvTransposeS2`, `NeighborhoodAttentionS2`, and their distributed counterparts is now derived from the grid's actual latitude spacing rather than from `nlat` alone. The previous expression was exact only for equiangular grids and under-covered the poles on the others. Equiangular grids, which are the default, are unaffected in practice; on the other grids the stencil changes, so models trained on them will differ unless `theta_cutoff` is passed explicitly. A `UserWarning` is emitted on the grids whose default changed.
