...
* **Breaking**: `DistributedDiscreteContinuousConvS2` gains `polar_mode`, defaulting to `"halo-exchange"`: each polar rank borrows a latitude halo and computes only the output rows it owns, so the K-expanded intermediate becomes `(B, C / azimuth_size, K, nlat_out / polar_size, nlon_out)` where its `nlat_out` axis previously scaled with nothing. Results are unchanged. `polar_mode="reduce-scatter"` restores the old behaviour and is the only option when the support spans more than the neighbouring rank — that case now raises instead of silently falling back, so a wide `theta_cutoff` that used to build may need the mode set explicitly. The transpose convolution is unaffected.
