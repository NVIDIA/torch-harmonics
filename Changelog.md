...
* **Breaking**: `DistributedDiscreteContinuousConvS2` gains `polar_mode`, defaulting to `"halo-exchange"`. Each polar rank now borrows the input latitudes its own output latitudes reach into and computes only the rows it owns, instead of computing every output latitude and completing the sum with a reduce-scatter. The K-expanded intermediate therefore becomes `(B, C / azimuth_size, K, nlat_out / polar_size, nlon_out)`: previously the `nlat_out` axis scaled with nothing, so adding polar ranks could not reduce it, which is what puts km-scale models out of memory during the step rather than during setup. Results are unchanged. Pass `polar_mode="reduce-scatter"` for the previous behaviour; it places no bound on the angular reach, which the halo does. A cutoff whose support spans more than the neighbouring rank now raises at construction and names that alternative, rather than silently selecting it — so a configuration that used to build may now require an explicit choice. The transpose convolution is unaffected and keeps its all-gather.
* Added `polar_halo_reduce`, the adjoint of `polar_halo_exchange`, for operators that scatter across the polar boundary rather than gather across it. Both directions were already implemented, but only as the forward and backward of one autograd function, so the accumulating direction could not be used as a forward. Also exports `compute_polar_halo_radius`.
