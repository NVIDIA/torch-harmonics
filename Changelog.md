...
* **Breaking**: the default `theta_cutoff` of `DiscreteContinuousConvS2`, `DiscreteContinuousConvTransposeS2`, `NeighborhoodAttentionS2`, and their distributed counterparts is now derived from the grid's actual latitude spacing rather than from `nlat` alone. The previous expression was exact only for equiangular grids and under-covered the poles on the others. Equiangular grids are unaffected; `"lobatto"` widens by ~21% and `"legendre-gauss"` narrows by ~2.3%, while for `"equiangular-trapezoidal"` the widening grows with resolution. Models trained on non-equiangular grids will differ unless `theta_cutoff` is passed explicitly.
...
* `truncate_support` joins `truncate_sht` in `torch_harmonics.truncation`, following a review suggestion on the theta-cutoff work that the angular cutoff is truncation logic and belongs beside the spectral kind. It is the spatial counterpart: given a grid it returns the support radius of a DISCO convolution or of neighborhood attention, applying an explicit override, rejecting any non-positive result, and warning when the default it picks differs from the pre-v0.9.3 heuristic. The positivity check is on the radius returned rather than on the argument it came from, so a non-positive `scale` on the default is rejected as an explicit non-positive `theta_cutoff` is. The five layers that each carried their own copy of that `if theta_cutoff is None` block and its positivity check now call it. `GridS2.theta_cutoff()` stays, reduced to `scale * max_latitude_spacing`: a fact about the node distribution, silent, which is what the descriptor promises everywhere else. `truncate_sht` rejects a shard through `require_grid`, and so does `truncate_support` wherever it derives the radius from the grid, since a support radius taken from a shard's own spacing would differ between ranks -- a correctness bug and not an inefficiency. Passing a shard together with an explicit `theta_cutoff` remains allowed: nothing is then derived from the grid, and distributed callers holding only a shard can still ask for a radius they already know.
...
* Every distributed layer now obtains its spatial decomposition from `GridS2.shard()` instead of computing it itself. `DistributedQuadratureS2`, `DistributedResampleS2`, the four distributed SHTs, both distributed convolutions and `DistributedNeighborhoodAttentionS2` previously called `compute_split_shapes` on `nlat` and `nlon` directly, twenty-seven times in total; no call in `torch_harmonics.distributed` splits a grid dimension any more. What remains of `compute_split_shapes` there splits something that is not geometry: channel counts, the `l_shapes`/`m_shapes` spectral bounds, which stay manual because `lmax` and `mmax` are truncation limits rather than axes of the grid, and the generic tensor primitives, which partition whichever axis they are handed. `DistributedSpectralConvS2` needed no change, since it delegates to its SHT, inverse SHT and quadrature and only ever sees data already in spectral space. This is the change that makes a reduced Gaussian grid tractable: latitudes per rank stop being a function of `nlat` alone once rows carry different numbers of longitudes, and that now has one definition instead of twenty-seven.
* `DistributedQuadratureS2` builds only this rank's quadrature weights, where it previously built the global `(nlat, nlon)` tensor on every rank and sliced its own piece out of it. A point's weight does not depend on its longitude, so the local construction is exact rather than an approximation of the old one; the result is bit-identical.
