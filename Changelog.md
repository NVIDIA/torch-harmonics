...
* **Breaking**: the grid descriptors gained an intermediate class. `GridS2` now describes a stack of latitude rings and carries no `nlat`/`nlon`; those, along with `max_azimuthal_order`, `lon_shapes` and the 2D `shard(polar=, azimuth=)` decomposition, moved to the new `RegularGridS2`, which every implemented grid subclasses. `GridShardS2` split the same way, with the polar/azimuth decomposition moving to `RegularGridShardS2`. Existing code is unaffected unless it referenced `GridS2` in an `isinstance` check or a type annotation. This makes a ragged grid -- reduced Gaussian, HEALPix -- an additive change rather than a second API break.
* New `require_regular_grid` guards every routine that addresses a field as a dense `(nlat, nlon)` array with a uniform longitude stride, which is all of them today. Handing such a routine a ragged grid would not raise but silently index the wrong points, so the assumption is now stated once per routine and fails at construction. Each guard can be relaxed to `require_grid` independently as a backend gains support.
